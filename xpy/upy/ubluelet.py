...
def prepare(pass_root_coro):
    ...
    #UPY history = weakref.WeakKeyDictionary({root_coro: None})
...
def launcher():
    while not RunTime.wantQuit:
        ...
        while RunTime.CoRoutines:
            ...
            if has_self:
                print("BL_coro_launcher: Instance.CoRoutine : %s" % coro )
                yield spawn( coro() )
            else:
                print("Begin: Module.CoRoutine : %s" % coro )
                yield spawn( coro( __import__( has_module ) ) )
            print("BL_coro_launcher: CoRoutine : %s" % coro )
