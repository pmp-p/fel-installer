...
class SendEvent(WaitableEvent):
    ...
    def fire(self):
        if not isinstance(self.data,bytes):
            print(len(self.data))
            self.data = bytes( self.data , 'utf8' )
        if self.sendall:
...
def advance_thread(coro, value, is_exc=False):
    ...
    try:
        ...
        del threads[coro]
        tb = None
        #FIXME: maybe implement sleep/resume instead of ugly exit
        try:
            tb=sys.exc_info()
            RunTime.debugger.unhandled_coroutine( tb )
        except Exception as error:
            print('\033[33m  ** debugger connection failed with error [%s] **\033[0m'%repr(error))
            raise ThreadException(coro, tb)
        finally:
            del tb
...
class ASync:
    ...
    def sync(self,*argv,**kw):
        self.mark += 1
        self.write(*argv,**kw)
        mark = self.mark
        Time.sleep(0.002)
        self.read(mark)
        return self.__class__.Q.pop( mark )
    ...
    def __call__(self,*argv,**kw):
        ...
        self.write(*argv,**kw)
        mark = self.mark
        while not self.read(mark):
            yield bluelet.null()
        yield bluelet.end(value= self.value(mark) )

def launcher():
    while not RunTime.wantQuit:
        yield null()
        while RunTime.CoRoutines:
            yield null()
            coro = RunTime.CoRoutines.pop(0)
            try:
                has_self = coro.__self__
            except:
                has_self = None
            try:
                has_module = coro.__module__
            except:
                has_module = '__main__'
            #print("bluelet launch coro=%s  self=%s module=%s" % (coro,has_self,has_module) )
            if has_self:
                warn("BL_coro_launcher: Instance.CoRoutine : %s" % coro )
                yield spawn( coro() )
            else:
                warn("Begin: Module.CoRoutine : %s" % coro )
                yield spawn( coro( __import__( has_module ) ) )
            warn("BL_coro_launcher: CoRoutine : %s" % coro )
    print("BL_coro_launcher clean exit")



#
#def bluelet_run_once():
#    if bluelet.threads:
#        bluelet.run()
#        if bluelet.exit_te:
#            use.ns.err("exiting on error %s",bluelet.exit_te)
#            return use.exit()
