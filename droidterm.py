...
class Miniterm(object):
    ...
    def _write(self,data):
        if isinstance(data,str):
            return self.serial.write( bytes(data,"utf8") )
        ...
        try:
            while self.alive:
                ...
                if menu_active:
                    ...
                    self._write(self.newline)         # send newline character(s)
                else:
                    self._write(b)                    # send byte
        except:
            self.alive = False
            raise
...
3: hex dump everything""",
    ...
    port = TTY or options.port
    baudrate = options.baudrate
    if args and TTY is None:
